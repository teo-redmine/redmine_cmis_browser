...
class DmsfZip
  ...
  def add_file_s3(file, path)
    ...
    @zip_file.put_next_entry(string_path)
    attachment = RedmineS3::Connection.get(file.disk_filename)
  ...
  def add_folder_s3(folder, path)
    ...
    @zip_file.put_next_entry(string_path)
    folder.subfolders.each { |subfolder| self.add_folder_s3(subfolder, string_path) }
    folder.attachments.each { |file| self.add_file_s3(file, string_path) if file }
  end

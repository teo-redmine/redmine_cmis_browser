...
class DmsfController < ApplicationController
  ...
  def show
    ...
    # stub
#    @folder = RedmineS3::Folder.new(@project.cmis_object_id) unless @folder
    ...
    @ajax_upload_size = Setting.plugin_redmine_cmis_browser['dmsf_max_ajax_upload_filesize'].present? ? Setting.plugin_redmine_cmis_browser['dmsf_max_ajax_upload_filesize'] : 100
  ...
  def entries_operation
    ...
    flash[:error] = l(:error_max_files_exceeded, :number => Setting.plugin_redmine_cmis_browser['dmsf_max_file_download'])
    redirect_to :back
  rescue EmailMaxFileSize
    flash[:error] = l(:error_max_email_filesize_exceeded, :number => Setting.plugin_redmine_cmis_browser['dmsf_max_email_filesize'])
    redirect_to :back
  rescue FileNotFound
    render_404
  ...
  def email_entries(selected_folders, selected_files)
    begin
      ...
      max_filesize = Setting.plugin_redmine_cmis_browser['dmsf_max_email_filesize'].to_f
  ...
  def zip_entries(zip, selected_folders, selected_files)
    ...
    if selected_folders && selected_folders.is_a?(Array)
      selected_folders.each do |selected_folder_id|
        folder = RedmineS3::Folder.new(selected_folder_id)
        
        zip.add_folder_s3(folder, nil) if folder
      end
    end
    ...
    if selected_files && selected_files.is_a?(Array)
      selected_files.each do |selected_file_id|
        ...
        zip.add_file_s3(file, nil) if file
      end
    end
    max_files = Setting.plugin_redmine_cmis_browser['dmsf_max_file_download'].to_i
...
#  end
  ...
  def find_folder
    @folder = RedmineS3::Folder.new(params[:folder_id].present? ? params[:folder_id] : @project.cmis_object_id)
  rescue DmsfAccessError
    render_403

...
run_tests()
{
  ...
  bundle exec rake redmine:plugins:test:units NAME=redmine_cmis_browser
  bundle exec rake redmine:plugins:test:functionals NAME=redmine_cmis_browser
  bundle exec rake redmine:plugins:test:integration NAME=redmine_cmis_browser
}
...
uninstall()
{
  ...
  # clean up database
  bundle exec rake $MIGRATE_PLUGINS NAME=redmine_cmis_browser VERSION=0 RAILS_ENV=test  
}
...
run_install()
{
  ...
  ln -sf $PATH_TO_DMSF $PATH_TO_PLUGINS/redmine_cmis_browser
